...
class LentaPostsTableViewCell: UITableViewCell {
    ...
    private lazy var buttonThreePoint: UIButton = {
        let action = UIAction { _ in
            Task {
                guard let currentUser = await UserCoreDataManager.shared.getCurrentUser() else {return}
                guard let publication = self.currentPublication else {return}
                guard let newPublications = currentUser.publications else { return }
                for publicatione in newPublications where publicatione.id == publication.id {
                    let alertController = UIAlertController(title: nil, message: nil, preferredStyle: .actionSheet)
                    let deleteAction = UIAlertAction(title: "Удалить пост", style: .destructive) { _ in
                        PublicationsCoreDataManager.shared.asyncDelete(publication)
                    }
    ...
    func updateCountLikes(publicationId: UUID) {
        let count = PublicationsCoreDataManager.shared.getCountLikes(forPublicationId: publicationId)
    ...
    func configure(with publication: Publication) {
        ...
        datePublication.text = dateFormatter.string(from: publication.date ?? Date())
        descriptionPublication.text = publication.descriptions
        setImageHeart()
